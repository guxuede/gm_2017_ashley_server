...
public class MovementSystem extends IteratingSystem {
    ...
    public MovementSystem() {
        super(Family.all(PlayerDataComponent.class, PositionComponent.class).get());
        this.priority = 1;
    }
    ...
    public void processEntity(Entity entity, float deltaTime) {
        ...
        getNextPositionDelta(deltaTime, playerDataComponent, tmp);
        PositionComponent positionComponent = Mappers.positionCM.get(entity);
        positionComponent.position.add(tmp);

        //更新方向信息
        playerDataComponent.isMoving = tmp.x != 0 || tmp.y!=0;
        if (playerDataComponent.isMoving){
            positionComponent.degrees = tmp.angle();
            playerDataComponent.direction = convertDegreesToDirection(tmp.angle());
        }

        syncNetPosition(playerDataComponent, positionComponent);
    }

    public static void getNextPositionDelta(float deltaTime, PlayerDataComponent actorStateComponent, Vector2 out) {
        //加速度驱使速度
...
//        actorStateComponent.velocity.y = actorStateComponent.velocity.y* friction;
    ...
    private void syncNetPosition(PlayerDataComponent playerDataComponent, PositionComponent positionComponent) {
        if (positionComponent != null && !positionComponent.position.epsilonEquals(positionComponent.lastPositionReported)
        && (System.currentTimeMillis() - positionComponent.lastTimePositionReported) > 1000) {
            positionComponent.lastTimePositionReported = System.currentTimeMillis();
            positionComponent.lastPositionReported.set(positionComponent.position);
            PlayerPositionPack playerPositionPack = new PlayerPositionPack(playerDataComponent.getId(), playerDataComponent.direction, positionComponent.position);
            getEngine().getSystem(MessageOutboundSystem.class).broadCaseMessage(playerPositionPack);
        }
    }
